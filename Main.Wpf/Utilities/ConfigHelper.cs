...
namespace Main.Wpf.Utilities
{
    public static class ConfigHelper
    {
        ...
        public static bool _configIsChanging;
        ...
        public static void CreateConfigWatcher()
        {
            try
            {
                ...
                {
                    var path = Path.GetDirectoryName(Config.File);
        ...
        {
            if (_configIsChanging) return;

            LogFile.WriteLog("Config File change detected");

            _configIsChanging = true;

            await Application.Current.Dispatcher.BeginInvoke(DispatcherPriority.Background, new Action(async () =>
            {
                await Read().ConfigureAwait(false);

                if (int.TryParse(await XmlHelper.ReadString(Config.File, "selectionIndex").ConfigureAwait(false), out var index) && index - 1 >= 0)
